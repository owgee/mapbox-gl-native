...
#import <mapbox/geometry/geometry.hpp>
...
public:
    ...
    id <MGLFeature> operator()(const mapbox::geometry::point<T> &geometry) const {
    ...
    id <MGLFeature> operator()(const mapbox::geometry::line_string<T> &geometry) const {
    ...
    id <MGLFeature> operator()(const mapbox::geometry::polygon<T> &geometry) const {
    ...
    id <MGLFeature> operator()(const mapbox::geometry::multi_point<T> &geometry) const {
        std::vector<CLLocationCoordinate2D> coordinates;
        coordinates.reserve(geometry.size());
        std::transform(geometry.begin(), geometry.end(), std::back_inserter(coordinates), coordinateFromPoint);
    ...
    id <MGLFeature> operator()(const mapbox::geometry::multi_line_string<T> &geometry) const {
    ...
    id <MGLFeature> operator()(const mapbox::geometry::multi_polygon<T> &geometry) const {
    ...
    id <MGLFeature> operator()(const mapbox::geometry::geometry_collection<T> &collection) const {
...
- (NS_ARRAY_OF(id <MGLFeature>) *)visibleFeaturesAtPoint:(NSPoint)point {
    ...
    mbgl::ScreenCoordinate flippedPoint = { point.x, NSHeight(self.bounds) - point.y };
    std::vector<mbgl::Feature> features = _mbglMap->queryRenderedFeatures(flippedPoint);
    std::vector<id <MGLAnnotation>> annotations;
    annotations.reserve(features.size());
    std::transform(features.begin(), features.end(), std::back_inserter(annotations),
                   ^id <MGLFeature> (const mbgl::Feature &feature) {
